    ...
    case $opt in
        d)
            ...
            then
                ...
                echo "  error : db must be redis or mongodb" 
                usage 
                exit 0
            fi           
            ;;
...
perf stat -e ${PMU_D} -o ${DIR_OUTPUT_PERF}/${DB}-${HP_TYPE}-${M_FRG}_${SZ}.dat -a ./bin/ycsb run ${DB} -P ${DIR_DB_CONF}/${DB}.prop -P ${DIR_WORKLOAD}/${TYPE_WR} -threads `nproc` -s > ${DIR_OUTPUT_OPT}/${DB}-${HP_TYPE}-${M_FRG}_${SZ}.dat 

source ./_check.sh > ${DIR_OUTPUT_SYS}/${DB}-${HP_TYPE}-${M_FRG}_${SZ}_after.dat  

sleep 2
source /home/son/_pid_check.sh -n mongod >> ${DIR_OUTPUT_SYS}/${DB}-${HP_TYPE}-${M_FRG}_${SZ}_after.dat 
